...
interface EstimateSettingListProps {
  estimates: Estimate[];
  ...
  onAccordionChange: (estimateId: number | string) => (event: React.SyntheticEvent, isExpanded: boolean) => void;
  onDeleteEstimate: (estimateId: number) => void;
  onMainStoreChange: (estimateId: number, newStores: string[]) => void;
  onCoStoreChange: (estimateId: number, newStores: string[]) => void;
  onItemEditStart: (estimateId: number, itemId: number, currentName: string) => void;
  onItemEditCancel: (estimateId: number, itemId: number) => void;
  onItemEditSave: (estimateId: number, itemId: number) => void;
  onItemNameChange: (estimateId: number, itemId: number, value: string) => void;
}
...
export default function EstimateSettingList({
  estimates,
  availableStores,
  expandedAccordions,
  editingItems,
      ...
      {Object.entries(groupedEstimates).map(([agencyName, agencyEstimates]) => (
        <Box key={agencyName} sx={{ mb: 4 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {agencyEstimates.map((estimate) => (
              <Accordion
                key={estimate.id}
                expanded={expandedAccordions[estimate.id] || false}
                onChange={onAccordionChange(estimate.id)}
                sx={{
                    ...
                    <Grid container spacing={3} sx={{ flexGrow: 1 }}>
                      <Grid item xs={12} sm={6} md={3}>
                        ...
                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, alignItems: 'center' }}>
                          {estimate.mainStoreNames.map((store, index) => (
                            <Chip
                              key={index}
                              label={store}
                              size="small"
                              ...
                              onDelete={(e) => {
                                e.stopPropagation();
                                const newStores = estimate.mainStoreNames.filter((_, i) => i !== index);
                                onMainStoreChange(estimate.id, newStores);
                              }}
                            />
                          ))}
                          <FormControl size="small" sx={{ minWidth: 120 }}>
                            <Select
                              ...
                              onChange={(e) => {
                                e.stopPropagation();
                                if (e.target.value && !estimate.mainStoreNames.includes(e.target.value as string)) {
                                  onMainStoreChange(estimate.id, [...estimate.mainStoreNames, e.target.value as string]);
                                }
                              }}
                              displayEmpty
                              renderValue={() => '追加'}
                            ...
                            >
                              {availableStores
                                .filter(store => !estimate.mainStoreNames.includes(store))
                                .map((store) => (
                      ...
                      <Grid item xs={12} sm={6} md={3}>
                        ...
                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, alignItems: 'center' }}>
                          {estimate.coStoreNames.map((store, index) => (
                            <Chip
                              key={index}
                              label={store}
                              size="small"
                              ...
                              onDelete={(e) => {
                                e.stopPropagation();
                                const newStores = estimate.coStoreNames.filter((_, i) => i !== index);
                                onCoStoreChange(estimate.id, newStores);
                              }}
                            />
                          ))}
                          <FormControl size="small" sx={{ minWidth: 120 }}>
                            <Select
                              ...
                              onChange={(e) => {
                                e.stopPropagation();
                                if (e.target.value && !estimate.coStoreNames.includes(e.target.value as string)) {
                                  onCoStoreChange(estimate.id, [...estimate.coStoreNames, e.target.value as string]);
                                }
                              }}
                              displayEmpty
                              renderValue={() => '追加'}
                            ...
                            >
                              {availableStores
                                .filter(store => !estimate.coStoreNames.includes(store))
                                .map((store) => (
                                  <MenuItem key={store} value={store}>
                                    {store}
                                  </MenuItem>
                  ...
                  <Table size="small">
                    ...
                    <TableBody>
                      {estimate.items.map((item) => {
                        const editingKey = `${estimate.id}-${item.id}`;
                          ...
                          <TableRow key={item.id} sx={{ height: '53px' }}>
                            ...
                            <TableCell sx={{ width: '200px', minWidth: '200px', padding: '8px' }}>
                              {isEditing ? (
                                <TextField
                                  value={editingValue}
                                  onChange={(e) => onItemNameChange(estimate.id, item.id, e.target.value)}
                                  size="small"
                                  variant="outlined"
                                  fullWidth
                                  autoFocus
                                  ...
                                  onKeyDown={(e) => {
                                    if (e.key === 'Enter') {
                                      onItemEditSave(estimate.id, item.id);
                                    } else if (e.key === 'Escape') {
                                      onItemEditCancel(estimate.id, item.id);
                                    }
                                  }}
                                />
                              ) : (
                              ...
                              {isEditing ? (
                                <Box sx={{ 
                                  ...
                                  <Button
                                    size="small"
                                    variant="contained"
                                    onClick={() => onItemEditSave(estimate.id, item.id)}
                                    sx={{ 
                                      fontSize: '0.7rem',
                                      width: '70px',
                                      height: '20px',
                                  ...
                                  <Button
                                    size="small"
                                    variant="outlined"
                                    onClick={() => onItemEditCancel(estimate.id, item.id)}
                                    sx={{ 
                                      fontSize: '0.7rem',
                                      width: '70px',
                                      height: '20px',
                                ...
                                <Button
                                  size="small"
                                  variant="text"
                                  onClick={() => onItemEditStart(estimate.id, item.id, item.itemName)}
                                  sx={{ 
                                    fontSize: '0.7rem',
                                    minWidth: '50px',
                                    height: '24px',
                      ...
                      <TableRow>
                        ...
                        <TableCell align="right" sx={{ fontWeight: 'bold', borderTop: '2px solid #e0e0e0', backgroundColor: '#f5f5f5', fontSize: '1.1rem', width: '100px', minWidth: '100px' }}>
                          ¥{(estimate.items.reduce((sum, item) => sum + item.amount, 0) + 
                             Math.floor(estimate.items
                               .filter(item => item.taxType === 'taxable')
                               .reduce((sum, item) => sum + item.amount, 0) * 0.1))
                             .toLocaleString()}
