...
| ログアウト実行     | ボタン押下時   | ログアウト処理を実行                 | /tier-2dealer/admin/login | なし          |
...
| 要素名             | コントロール種類 | I/O | 説明                                                                 |
|--------------------|------------------|-----|----------------------------------------------------------------------|
| スタッフ一覧       | テーブル         | 出力| スタッフ情報（氏名・カナ・役職・単価・電話番号・提出状況等）         |
| 提出状況バッジ     | バッジ           | 出力| 選択月の提出状況を「提出済み」「未提出」で色分け表示                |
| アクティブ切替     | スイッチ         | 入力| スタッフの有効/無効状態切替                                         |
| パスワード表示     | アイコンボタン   | 入力| パスワードの表示/非表示切替                                         |
...
| パスワード表示切替 | クリック時     | パスワードの表示状態切替   | なし   | なし           |
...
| 平日単価入力欄     | 数値入力         | 入力| 平日の日当を入力                                                     |
| 土日単価入力欄     | 数値入力         | 入力| 土日の日当を入力                                                     |
| 電話番号入力欄     | テキストボックス | 入力| スタッフの電話番号を入力                                             |
| 役職選択           | セレクトボックス | 入力| クローザー/ガール等を選択                                            |
| 所属会社入力欄     | テキストボックス | 入力| 所属会社名を入力                                                     |
| メール入力欄       | テキストボックス | 入力| メールアドレスを入力                                                 |
| パスワード入力欄   | テキストボックス | 入力| ログインパスワードを入力                                             |
| 出張可否選択       | セレクトボックス | 入力| 出張対応可否を選択                                                   |
| 提出状況選択       | セレクトボックス | 入力| 選択月の提出状況を「提出済み」「未提出」から設定                     |
| アクティブ切替     | スイッチ         | 入力| スタッフの有効無効状態を設定                                         |
...
| 要素名             | コントロール種類 | I/O | 説明                                                                 |
|--------------------|------------------|-----|----------------------------------------------------------------------|
| 確認メッセージ     | テキスト表示     | 出力| 削除対象スタッフ名と確認メッセージ                                   |
| 削除実行ボタン     | ボタン           | 入力| 削除を実行                                                           |
...
- **LocalStorage管理**: スタッフ情報・提出状況・通知データの永続化
- **リアルタイムフィルタ**: 入力と同時にテーブル内容を絞り込み
- **提出状況管理**: 月別の提出状況を個別管理・表示（「提出済み」「未提出」バッジで日本語表示）
- **アクティブ状態管理**: スタッフの有効/無効状態をリアルタイム切替
- **パスワード管理**: パスワードの表示/非表示切替機能
- **バリデーション**: 必須項目（氏名、カナ、電話番号、メール、パスワード）、形式チェック（メールアドレス、電話番号）、数値チェック（平日・土日単価）
...
- **通知機能**: リアルタイム通知表示、未読/既読状態管理、通知タイプ別分類
