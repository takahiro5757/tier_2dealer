...
| 要望セル           | RequestCell      | 入力  | スタッフの要望数入力（整数のみ、1-30の範囲制限）                     |
...
| 要望入力           | 数値入力時     | 要望数の保存（整数のみ、1-30の範囲）  | なし                | 数値・範囲チェック |
...
## 2-6. 要望セル（RequestCell）数値入力機能

### UI 要素・コントロール・I/O

| 要素名             | コントロール種類 | I/O   | 説明                                                                 |
|--------------------|------------------|-------|----------------------------------------------------------------------|
| 要望セル           | RequestCell      | 入力  | スタッフの要望数入力セル（紫背景・中央配置）                         |
| 要望数表示         | Typography       | 表示  | 現在の要望数を表示（クリックで編集モード）                           |
| 編集テキストフィールド | TextField    | 入力  | 数値入力専用のテキストフィールド（編集モード時）                     |

### 数値入力機能仕様

#### 入力制限
- **入力形式**: 整数のみ（小数点・文字列は無効）
- **入力範囲**: 1-30の範囲制限
- **無効値処理**: 範囲外・無効値の場合はデフォルト値（15）を設定
- **入力モード**: `inputMode: 'numeric'`、`pattern: '[0-9]*'`で数値キーパッドを表示

#### 編集操作
- **編集開始**: セルクリックで編集モード開始（編集権限がある場合のみ）
- **編集確定**: 
  - Enter キー押下
  - フィールド外クリック（Blur）
- **編集キャンセル**: Escape キー押下
...
#### 表示仕様
- **セルサイズ**: 幅60px・高さ36px固定
- **背景色**: #f3e5f5（薄紫）・hover時 #e1bee7
- **文字色**: #9c27b0（紫）
- **フォント**: 14px・太字・中央揃え
- **プレースホルダー**: 「整数」

#### データ管理
- **デフォルト値**: 15（データなし・無効値の場合）
- **古いデータ変換**: 文字列形式の古いデータは自動的に15に変換
- **保存形式**: `request`フィールドに整数で保存
- **編集権限**: `onRequestTextChange`コールバックが提供されている場合のみ編集可能

### イベント定義

| イベント名         | 発生タイミング | イベント概要                         | 遷移先/動作         | バリデーション |
|--------------------|---------------|--------------------------------------|---------------------|---------------|
| 要望編集開始       | セルクリック時 | 編集モード開始・フォーカス設定       | 編集モード          | 編集権限チェック |
| 要望数保存         | Enter・Blur時  | 数値検証・保存・編集モード終了       | 表示モード          | 数値・範囲チェック |
| 要望編集キャンセル | Escape時       | 変更内容破棄・編集モード終了         | 表示モード          | なし          |
| 数値入力           | 入力時         | 数字のみ許可（正規表現チェック）     | なし                | 数字のみ許可  |

---

## 2-7. シフトセル選択メニュー（StatusCell）
...
#### スタッフ別変更内容表示
- **表示形式**: スタッフ名見出し + 変更項目リスト
- **スタッフ名**: 太字で表示
- **変更項目**: インデント付きで各変更項目を表示

#### シフト希望変更
- **表示条件**: シフトステータス（○×△）の変更がある場合のみ
- **表示形式**: 「MM月DD日: 変更前 → 変更後 (シフト希望)」
- **例**: 「6月12日: ○ → × (シフト希望)」

#### 要望変更
- **表示条件**: 要望数（request）の変更がある場合のみ
- **表示形式**: 「変更前数値 → 変更後数値 (要望)」
- **例**: 「15 → 18 (要望)」
...
| ステータスバッジ   | Chip             | 出力  | pending（承認待ち）/answered（回答済み）状態表示                     |
...
| ステータスバッジ   | Chip             | 出力  | 承認待ち・回答済みの状態をカラー表示                                 |
| スタッフ別変更内容 | 変更内容表示     | 出力  | スタッフ名見出し + 各変更項目の詳細表示                              |
| シフト希望変更表示 | 変更項目表示     | 出力  | 「MM月DD日: 変更前 → 変更後 (シフト希望)」形式で表示                |
| 要望変更表示       | 変更項目表示     | 出力  | 「変更前数値 → 変更後数値 (要望)」形式で表示                         |
| 日付表示変換       | 日付変換         | 出力  | YYYY-MM-DD形式をMM月DD日形式に変換表示                              |
| 変更前後比較       | 比較表示         | 出力  | 「変更前 → 変更後」形式で変更内容を明示                              |
| コメント履歴セクション | 履歴表示     | 出力  | 変更依頼に関するコメント履歴を時系列で表示                           |
| コメント履歴リスト | リスト表示       | 出力  | 承認者・コメント内容・タイムスタンプを表示                           |
| コメント作成者表示 | テキスト表示     | 出力  | コメント作成者名（太字・青色で強調表示）                             |
| コメント日時表示   | 日時表示         | 出力  | コメント作成日時（YYYY/MM/DD HH:MM形式）                            |
| コメント内容表示   | テキスト表示     | 出力  | コメント本文の表示                                                   |
| 空コメント表示     | メッセージ表示   | 出力  | コメントがない場合は「コメントはありません」を中央表示               |
...
#### 表示構成
- **セクションタイトル**: 「コメント履歴」（コメントアイコン付き）
- **コンテナ**: 最大高さ300px・スクロール可能・境界線付き
- **背景**: 白色・角丸・内側パディング付き

#### コメント項目表示
- **作成者名**: 太字・青色（primary.main）で表示
- **作成日時**: 右寄せ・YYYY/MM/DD HH:MM形式・グレー文字
- **コメント内容**: 通常文字・左寄せ
- **区切り線**: コメント間に薄いグレーの境界線

#### データ管理
- **データソース**: `selectedRequest.commentHistory`配列から取得
- **並び順**: タイムスタンプの古い順（配列順）で表示
- **空データ処理**: コメントがない場合は「コメントはありません」を中央表示

#### 表示条件
- **表示対象**: `commentHistory`配列が存在し、要素数が1以上の場合
- **非表示条件**: `commentHistory`が未定義またはnull、空配列の場合は空メッセージ表示

---

## 2-14. 通知システム（NotificationSystem）
