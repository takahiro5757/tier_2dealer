...
| 対象スタッフ       | テーブルセル     | 出力  | 変更対象スタッフ名（複数の場合は省略表示）                           |
| 対象月             | テーブルセル     | 出力  | 変更対象の年月                                                       |
| 依頼日時           | テーブルセル     | 出力  | 変更依頼の作成日時                                                   |
| ステータスバッジ   | バッジ           | 出力  | pending/approved/rejected状態表示                                    |
...
| 履歴更新           | 定期実行（5秒間隔） | 履歴データの再読み込み           | なし                | なし          |
...
| 変更項目           | テーブルセル     | 出力  | シフト希望・要望テキスト・平日希望数・土日希望数                     |
| 変更前後比較       | テーブルセル     | 出力  | 変更前の値と変更後の値を比較表示                                     |

### イベント定義

| イベント名         | 発生タイミング | イベント概要                         | 遷移先/動作         | バリデーション |
|--------------------|---------------|--------------------------------------|---------------------|---------------|
| ダイアログクローズ | ボタン押下時   | 詳細ダイアログを閉じる               | なし                | なし          |
...
- **変更依頼フロー**: 提出済み→変更依頼→承認待ち→承認/却下の状態管理。差分自動検出・解析、履歴管理、通知システム連携
